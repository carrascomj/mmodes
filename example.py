...
def main():
    ...
    cons = mmodes.Consortium(stcut = 1e-7, mets_to_plot = ["ac[e]", "thr_L[e]"], v = 1, manifest = "COMETS_manifest.txt")
    ...
    cons.add_model("../mmodes/ModelsInput/BGN4_eu.xml", 0.0003, solver = "glpk", method = "fba", dMets = {glc.id: glc}) # Bifidobacterium_adolescentis_ATCC_15703.xml (euavg)
    # 4) instantiate media
    cons.media = cons.set_media(media, True)
    # 5) run it, plotting the output
    cons.run(maxT = 10, outp = "plot_example.png", outf = "tab_example.tsv", verbose=True, integrator = "fea", stepChoiceLevel = (0.008,0.5,10000))
